...
export default function Detail() {
  ...
  useEffect(() => {
    axios
      .get(`http://localhost:7000/api/staff/${id}`)
      .then((res) => {
        setStaf(res.data.data);
      })
      .catch((err) => {
        console.error(err);
      });
        ...
        <div className="w-full flex flex-col justify-between">
          <main id="detail-staff-contents">
            <div className="rounded-sm border border-neutral-02 shadow-md m-5 px-5 py-5">
              <div className="text-lg font-bold mb-14">Detail Data Staff</div>
              <div className="flex space-x-8">
                ...
                <div className="divide-y divide-neutral-02 w-2/3">
                  <div className="flex w-full space-x-4 py-2">
                    ...
                    <div className="w-3/5">{staf.nama}</div>
                  </div>
                  <div className="flex w-full space-x-4 py-2">
                    <div className="w-2/5 font-semibold">NIP</div>
                    <div className="w-3/5">{staf.nip}</div>
                  </div>
                  <div className="flex w-full space-x-4 py-2">
                    <div className="w-2/5 font-semibold">Program Studi</div>
                    <div className="w-3/5">{staf.program_studi}</div>
                  </div>
                  <div className="flex w-full space-x-4 py-2">
                    <div className="w-2/5 font-semibold">Divisi</div>
                    <div className="w-3/5">{staf.divisi}</div>
                  </div>
                </div>
              </div>
            </div>
